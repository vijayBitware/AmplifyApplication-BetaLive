...
public class ChipsInputLayout extends MaxHeightScrollView
    ...
    TextView txtTagCnt, txtLimit,txtTagName, txtTagAddress;
    SharedPref pref;

    public CallBackInt callback = new CallBackInt() {
        @Override
        public void UpdateMyText(int cnt) {
            System.out.println("***chiplayout*****" + cnt);
            txtTagCnt.setText(String.valueOf(cnt));
        }
    ...
    public ChipsInputLayout(Context c, AttributeSet attrs) {
        ...
        System.out.println("*********"+SharedPref.getPreferences().getString(SharedPref.FIRST_NAME,""));
        Typeface face = Typeface.createFromAsset(c.getAssets(),
                "MyriadPro-Bold.otf");
        txtTagName.setText(SharedPref.getPreferences().getString(SharedPref.FIRST_NAME,""));
        txtTagAddress.setText("Pune");//(SharedPref.getPreferences().getString(SharedPref.USER_ADDRESS,""));
        txtTagName.setTypeface(face);
        txtTagAddress.setTypeface(face);
        txtTagCnt.setTypeface(face);
        txtLimit.setTypeface(face);
        // Setup the chips recycler view
        mChipsAdapter = new ChipsAdapter(
                mDataSource, loadChipsInput(), mOptions, callback);
